...
def simulate_heston_path(S0, T, r, kappa, theta, sigma, rho, nu0, n_steps=1000):
    dt = T / n_steps
    ...
    # Generate all Brownian motions at once
    dW1 = np.random.normal(0, np.sqrt(dt), n_steps)  # Brownian motion for price
    dW2 = np.random.normal(0, np.sqrt(dt), n_steps)  # Brownian motion for variance
    ...
    for t in range(1, n_steps + 1):
        prices[t] = prices[t - 1] * np.exp((r - 0.5 * variances[t - 1]) * dt + np.sqrt(variances[t - 1]) * dW1[t - 1])
        
        # Variance process (Heston's stochastic volatility equation)
        variances[t] = np.abs(variances[t - 1] + kappa * (theta - variances[t - 1]) * dt + sigma * np.sqrt(variances[t - 1]) * dW2[t - 1])

    return prices, variances

def heston_price(S0, K, T, r, kappa, theta, sigma, rho, nu0, n_samples=100, n_steps=1000):
    option_prices = []
    
    # Generate all paths at once using vectorized simulation
    paths = [simulate_heston_path(S0, T, r, kappa, theta, sigma, rho, nu0, n_steps) for _ in range(n_samples)]
    
    # Calculate option prices in parallel
    option_prices = [np.exp(-r * T) * max(path[0][-1] - K, 0) for path in paths]

    return np.mean(option_prices)

def generate_heston_test_data(S0, strikes, maturities, rate, kappa, theta, sigma, rho, nu0, n_samples=100):
    test_data = torch.zeros(len(maturities), len(strikes))

    for i, T in enumerate(maturities):
        test_data[i, :] = torch.tensor([heston_price(S0, K, T/365, rate, kappa, theta, sigma, rho, nu0, n_samples) for K in strikes])
...
class ModelTester:
    ...
    def run_comprehensive_test(self, S0, strikes, maturities, rate, kappa, theta, sigma, rho, nu0):
        """Run comprehensive testing using the Heston model"""
        # Generate independent test data using the Heston model
        test_data = generate_heston_test_data(S0, strikes, maturities, rate, kappa, theta, sigma, rho, nu0)
...
class Net_LV_Improved(nn.Module):
    ...
    def __init__(self, dim, timegrid, strikes_call,  n_layers, vNetWidth, device, rate, maturities, n_maturities):
        ...
        # Correctly instantiate the LSTM-based diffusion model
        self.diffusion = Net_LSTM_Grid(
            input_dim=dim + 1,  # Input dimension (e.g., [t, S])
            lstm_hidden_dim=50,  # Hidden dimension for LSTM
            lstm_layers=2,       # Number of LSTM layers
            output_dim=1,         # Output dimension
            n_maturities=n_maturities
        )

        # Correctly instantiate the LSTM-based control variates
...
def train_nsde_improved(model, z_test, config):
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 100 == 0:
            torch.save(error_hedge, "error_hedge_heston_nn.pth.tar")
        ...
        with open("training_errors_heston_nn.txt", "a") as f:
    ...
    plt.tight_layout()
    plt.savefig('final_error_curves_heston_nn.png')
    plt.close()

    # Save the error data to a numpy file for later analysis
    np.savez('training_errors_heston_nn.npz',
             epochs=epochs,
...
if __name__ == '__main__':
    ...
    training_data = generate_heston_test_data(
        S0=S0, 
    ...
    test_kappa = 2.2      # Slightly different parameters for testing
    test_theta = 0.045
    test_sigma = 0.32
    test_rho = -0.65
    test_nu0 = 0.045
    
    test_data = generate_heston_test_data(
        S0=S0, 
        strikes=strikes_call, 
        maturities=maturities, 
        rate=rate, 
    ...
    # Save test results
    def save_test_results(results, filename="test_results_heston_nn.txt"):
    ...
    def plot_results(results):
        ...
        plt.savefig('prediction_comparison_heston_nn.png')
        plt.close()
