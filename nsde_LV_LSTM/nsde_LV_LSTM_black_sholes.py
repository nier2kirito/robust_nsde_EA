...
def train_nsde_improved(model, z_test, config):
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 100 == 0:
            torch.save(error_hedge, "error_hedge_bs_nn.pth.tar")
        ...
        with open("training_errors_bs_nn.txt", "a") as f:
    ...
    plt.tight_layout()
    plt.savefig('final_error_curves_bs_nn.png')
    plt.close()

    # Save the error data to a numpy file for later analysis
    np.savez('training_errors_bs_nn.npz',
             epochs=epochs,
